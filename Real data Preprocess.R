...
legend("left", 
       legend = c("No fibers", "Fibers"), 
       fill = c("lightblue", "darkred"), 
       title = "Connections",
       x = -0.001,  # Adjust this value as needed
       y = 0.5,   # You might also need to adjust this
       xjust = 0, yjust = 0.5)
...
breaks <- seq(min(W_average, na.rm = TRUE), max(W_average, na.rm = TRUE), length.out = num_colors + 1)
legend_labels <- round(seq(min(W_average, na.rm = TRUE), max(W_average, na.rm = TRUE), length.out = num_colors), 2)
...
legend("left", legend = legend_labels, fill = color_scale, title = "Valori", xpd = TRUE)
